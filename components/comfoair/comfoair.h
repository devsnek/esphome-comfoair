...
class ComfoAirComponent : public climate::Climate,
 ...
 public:
  ...
  void dump_config() override {
    ...
    ESP_LOGCONFIG(TAG, "  Connector Board %.10s v%0d.%02d", p + 2, *p,
                  *(p + 1));

    if (*(p + 12) != 0) {
      ESP_LOGCONFIG(TAG, "  CC-Ease v%0d.%02d", *(p + 12) >> 4,
                    *(p + 12) & 0x0f);
    }
    if (*(p + 13) != 0) {
      ESP_LOGCONFIG(TAG, "  CC-Luxe v%0d.%02d", *(p + 13) >> 4,
                    *(p + 13) & 0x0f);
    }
    ...
    switch (data_[COMMAND_IDX_MSG_ID]) {
      ...
      case RES_GET_OPERATION_HOURS: {
        ...
        if (level3_hours != nullptr) {
          level3_hours->publish_state((msg[17] << 16) | (msg[18] << 8) |
                                      msg[19]);
        }
...
class ComfoAirButton : public button::Button {
 public:
  virtual void press_action() {
    if (parent_) {
      parent_->handle_action(get_name());
    }
  }
